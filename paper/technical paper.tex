...
\begin{document}
	...
	\section{Data}
		...
		Since there isn't an open-source dataset of statements with truth values, I'm going to collect this data by scraping trivia websites or articles that tell the user whether the sentence is true or not. When I have enough statements, ideally at least 500 instances, I'll use an API to enter the statement in a Google search and scrape the results for `snippets' to input into the dataset as well \cite{k_api}. Once added to the dataset, cleaning will include removing markdown commands.
